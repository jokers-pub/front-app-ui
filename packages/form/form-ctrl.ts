...
> {
    ...
    get contentAlign() {
        if (this.props.align) return this.props.align;

        let field = this.$root.closest(
            (n) => n instanceof VNode.Component && n.component.name === componentName("field")
        )?.component;
...
export function registerFormCtrl(com: Component<any>) {
    ...
    if (com.$root && com.$root instanceof VNode.Node) {
        ...
        let formItem = com.$root?.closest((n) => {
            return n instanceof VNode.Component && n.component.name === componentName("field");
        })?.component;
